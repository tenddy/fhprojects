...
@LastEditTime : 2020-01-19 10:07:02
        ...
        if 'ONU_LANPORT' in kwargs.keys():   # 配置端口业务配置
            for portno in range(onu.loc[index]['PORTNUM']):
                ...
                if kwargs['ONU_LANPORT'] == 5: # transparent+translate+qinq
                    cmdlines += OLT_V5.onu_lan_service((slotno,ponno,onu.loc[index]['ONU'], portno+1), 1, {'cvlan': ('transparent', tcos, cvlan_trans+index*cstep), 'translate': ('enable', tcos, tvlan+portno+index*tstep),'qinq':('enable', scos, svlan, "qinq_ippon", svlan_service)})
